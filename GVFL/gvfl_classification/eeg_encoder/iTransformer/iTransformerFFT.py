...
from old_retrieval.iTransformer import Attend
from old_retrieval.iTransformer import RevIN
...
class iTransformerFFT(Module):
    ...
    def forward(
        ...
        # project into variate tokens, for both time and fourier domains - fourier is spared the normalization

        x = self.mlp_in(x)

        x_fft = self.fft_mlp_in(x_fft)

        # place the fft tokens on the left, to be spliced out later
